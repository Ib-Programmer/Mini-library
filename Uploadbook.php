...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    ...
    if(($_FILES["pdfimage"]["size"] > 625000) && ($_FILES["pdfdocument"]["size"] > 3000000 )) {
     ...
     $booktitle = $bookauthor =  $bookyear = $booksubarea = $bookdisc  =  $bookimagepath = $bookpdfpath = "";

     $sql = $connection->prepare("INSERT INTO bookinformations(Booktitle,Bookauthor,
     Yearofpub,Subjectarea,Bookdisc,Bookimage,Bookpdf)
     VALUES (?,?,?,?,?,?,?)");
          ...
          $sql->bind_param("sssssss",$booktitle,$bookauthor,$bookyear,$booksubarea,$bookdisc,$bookimagepath,$bookpdfpath);
          if(empty($booktitle) || empty($bookauthor) || empty($bookyear) || empty($booksubarea) || empty($bookdisc)){
            ...
            if($uploadOk == 0){
              ...
              if( move_uploaded_file($_FILES["pdfimage"]["tmp_name"], $bookimage_file) &&
                move_uploaded_file($_FILES["pdfdocument"]["tmp_name"], $bookpdf_file)){
                  $sql->execute();
                  $success = "Book was  Successfully Uploaded";
                } else {
      ...
      $error = $error. "Sorry, you didn't select any file to upload ."; 
   }
}
   ?>
...
<form class="register" id="adminregister"  method="post"  enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
   ...
   <input type="text" class="input_field"  name="booktitle"><br>

   <label>Book Author  </label><br>
   <input type="text" class="input_field"  name="bookauthor"><br>

    <label>Year of Publication</label><br>
    <input type="text" class="input_field" name="yearofpublication"><br>

     <label>Subject Area </label><br>
     <input type="text" class="input_field" name="subjectarea"><br>

     <label>Book Discription</label><br>
     <input type="text" class="input_field" name="bookdiscription"><br>
    ...
    <input type="file" name="pdfdocument" id="pdfdocument"><br>
     ...
     <input type="submit"  id="btnadminreg" class="btn btn-primary" name="btnadminregister" value="Upload"><br>
