...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
       ...
       $regnumumber =  $name = $email = $phonenumber = $password = "";
       $privilege = 'Student';
       $stmt = $connection->prepare("INSERT INTO users(Regnum,Name,
      ...
      if(isset($_POST['btnadminregister'])){
           ...
           $password =  validate_input($_POST['password']);
            ...
            $stmt->bind_param("ssssss",$regnumber,$name, $email, $phonenumber,$hash,$privilege);
            if(empty($name) || empty($regnumber) || empty($email) || empty($phonenumber) || empty($password) ){
...
<form class="register" id="adminregister"  method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
   ...
   <input type="text" class="input_field"  name="regnumber"><br>
