...
<?php
 ...
 $id = $name = $email = $phonenumber = $password = $privilege = "";
 ...
 if($result->num_rows > 0){ 
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST"){
      ...
      $privilege =   validate_input($_POST['privilege']);
      if(empty($_POST['password'])){
         ...
         $sql = $connection->prepare("UPDATE users SET Name = ?,Email = ?,Phonenumber = ?,
         Privilege = ? WHERE Id = ? ");
        //binding the prepered statement
        $sql->bind_param('sssss',$name,$email,$phonenumber,$privilege,$id);   
      }
      else{
        ...
        //binding the prepered statement
        $sql->bind_param('ssssss',$name,$email,$phonenumber,$password,$privilege,$id);
      }
      if(empty($id) || empty($name) || empty($email) || empty($phonenumber) || empty($privilege)){
...
?>
 ...
 <form class="updateuser" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
   ...
   <input type="text" class="input_field" name="privilege" value="<?php echo $privilege; ?>"><br>
   <p>
